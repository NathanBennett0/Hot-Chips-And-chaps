...
public class FuzzTest {
  ...
  public void fuzzTest1() {
    ...
    while (!app.initializeDone()) {
      pause(app, 200); // While the main menu is booting, wait
    }
   ...
   * The hashmap has 4 values, key as a direction, val as number of times
   * explored
  ...
  public void intelligence(App app, long timer) {
    ...
    while (app.fuzzStarted()) {
      ...
      // Ensure that the updated X and Y given to us on the map is a valid
      // location of the board

      // Use intelligence to find the next direction depending on which level
      if (checkTimer(timer) || !app.runningGame) {
        break; // Checks if the timer has exceeded
      }
      if (app.getPhase().maze().getLevel().getLevel() == 1) {
        c.keyPressed(pickDirectionL1());
      } else {
        c.keyPressed(pickDirectionL2());
      }
      ...
      if (checkTimer(timer) || !app.runningGame 
          || app.updateLocationX() == -1 || app.updateLocationX() == -1) {
        break; // Checks if the timer has exceeded
      }
    }
  }
  ...
  public int pickDirectionL1() {
    ...
    // Randomly iterating (Map iterator is still somewhat ordered, so used
    // shuffle instead)
  ...
  public int generateKeycode(String dir) {
    ...
    int random = rand.nextInt(2); // Random number 0 or 1
